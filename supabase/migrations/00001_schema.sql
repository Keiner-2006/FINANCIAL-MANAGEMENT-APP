-- ============================================================
-- FINANZAS EN FAMILIA - Esquema de Base de Datos
...
-- 3. TABLA: obligaciones_financieras
...
-- metodos_pago
create policy "metodos_pago_select_own" on public.metodos_pago
  for select using (auth.uid() = user_id);
create policy "metodos_pago_insert_own" on public.metodos_pago
  for insert with check (auth.uid() = user_id);
create policy "metodos_pago_update_own" on public.metodos_pago
  for update using (auth.uid() = user_id);
create policy "metodos_pago_delete_own" on public.metodos_pago
  for delete using (auth.uid() = user_id);

-- obligaciones_financieras
create policy "obligaciones_select_own" on public.obligaciones_financieras
  for select using (auth.uid() = user_id);
create policy "obligaciones_insert_own" on public.obligaciones_financieras
  for insert with check (auth.uid() = user_id);
create policy "obligaciones_update_own" on public.obligaciones_financieras
  for update using (auth.uid() = user_id);
create policy "obligaciones_delete_own" on public.obligaciones_financieras
  for delete using (auth.uid() = user_id);

-- documentos_activos
create policy "documentos_select_own" on public.documentos_activos
  for select using (auth.uid() = user_id);
create policy "documentos_insert_own" on public.documentos_activos
  for insert with check (auth.uid() = user_id);
create policy "documentos_update_own" on public.documentos_activos
  for update using (auth.uid() = user_id);
create policy "documentos_delete_own" on public.documentos_activos
  for delete using (auth.uid() = user_id);
...
-- almuerzos_excluidos
create policy "almuerzos_excluidos_select_own" on public.almuerzos_excluidos
  for select using (auth.uid() = user_id);
create policy "almuerzos_excluidos_insert_own" on public.almuerzos_excluidos
  for insert with check (auth.uid() = user_id);
create policy "almuerzos_excluidos_delete_own" on public.almuerzos_excluidos
  for delete using (auth.uid() = user_id);
